...
### Stub generation
- New setting `pyjinhx.generateStubsOnSave` (default `true`): `.pyi` stubs are
  regenerated by `python -m pyjinhx.stubgen` every time a `.pjx` file is saved.
  Trust-gated — does not run in untrusted workspaces.
- New setting `pyjinhx.pythonPath` (default `""`): explicit Python interpreter path for
  stub generation; empty uses `python` on `PATH`.
