const INDENT = "  ";
const { formatHtmlStructure } = require("./format-html-structure");

function formatPyjinhxTemplate(text) {
  return formatHtmlStructure(formatSlottedPascalTags(text));
}

function readDoubleQuotedContent(text, openQuoteIndex) {
...
function isHtmlSlotValue(value) {
...
function splitHtmlFragments(text) {
  return text
    .replace(/></g, ">\n<")
    .split("\n")
    .map((line) => line.trim())
...
function formatInnerSlotContent(content, innerIndent) {
...
function formatSlottedPascalTags(text) {
  const replacements = [];
  let searchFrom = 0;
