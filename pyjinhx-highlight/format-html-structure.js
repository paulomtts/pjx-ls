...
function readQuotedString(text, startIndex, quote) {
...
function readJinja(text, startIndex, marker, endMarker) {
...
function tokenizeHtmlTemplate(text) {
  const tokens = [];
  let index = 0;
...
function jinjaDepthDelta(value) {
...
function hasElementChildren(tokens, startIndex) {
  let depth = 0;
...
function tryInlineElement(tokens, startIndex) {
  const openToken = tokens[startIndex];
  if (openToken.type !== "open") {
    return null;
  }
  ...
  for (let index = startIndex + 1; index < tokens.length; index += 1) {
    ...
    if (token.type === "close" && token.name === openToken.name) {
      return {
        endIndex: index,
        value: `${openToken.value}${inner}${token.value}`,
      };
    }
...
function formatHtmlStructure(text) {
